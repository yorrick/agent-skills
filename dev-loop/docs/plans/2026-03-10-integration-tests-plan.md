...
**Goal:** Create a single Python integration test script that runs the full dev-loop end-to-end headlessly and verifies the results.

**Architecture:** One script (`tests/test_integration.py`) that scaffolds a project, creates a GitHub repo, runs `claude -p` with the `/dev-loop` command, monitors the process, then verifies local artifacts and GitHub state. Uses `atexit` for cleanup.

**Tech Stack:** Python 3.10+ stdlib only (subprocess, pathlib, json, tempfile, atexit, time, threading). No test framework — script outputs PASS/FAIL and exits 0/1.

---

### Task 1: Create test script skeleton with setup and cleanup

**Files:**
- Create: `tests/test_integration.py`

**Step 1: Create the script with PEP 723 header, setup, and cleanup**
...
def scaffold_project(project_dir: Path) -> None:
    ...
    (project_dir / "src" / "calculator" / "__init__.py").write_text(
...
# --- Placeholders for remaining tasks ---

def run_dev_loop(project_dir: Path) -> subprocess.CompletedProcess[str]:
    """Run the dev-loop headlessly. Implemented in Task 2."""
    raise NotImplementedError


def verify_local(project_dir: Path) -> None:
    """Verify local artifacts. Implemented in Task 3."""
    raise NotImplementedError


def verify_github(repo_name: str) -> None:
    """Verify GitHub state. Implemented in Task 4."""
    raise NotImplementedError
...
**Step 2: Verify it runs (will hit NotImplementedError but structure is valid)**

Run: `cd /Users/yorrickjansen/work/claude-code-plugins/dev-loop && python3 -c "import ast; ast.parse(open('tests/test_integration.py').read()); print('Syntax OK')"`
Expected: "Syntax OK"

**Step 3: Run quality gates**

Run: `cd /Users/yorrickjansen/work/claude-code-plugins && uv run ruff check dev-loop/tests/test_integration.py && uv run pyright dev-loop/tests/test_integration.py`
Expected: PASS

**Step 4: Commit**

```bash
cd /Users/yorrickjansen/work/claude-code-plugins
git add dev-loop/tests/test_integration.py
git commit -m "feat: add integration test skeleton with setup and cleanup"
```

---

### Task 2: Implement run_dev_loop with process monitoring

**Files:**
- Modify: `tests/test_integration.py` (replace `run_dev_loop` placeholder)

**Step 1: Implement run_dev_loop**
...
cd /Users/yorrickjansen/work/claude-code-plugins
git add dev-loop/tests/test_integration.py
git commit -m "feat: implement run_dev_loop with process monitoring and timeout"
```

---

### Task 3: Implement verify_local

**Files:**
- Modify: `tests/test_integration.py` (replace `verify_local` placeholder)

**Step 1: Implement local verification**
...
cd /Users/yorrickjansen/work/claude-code-plugins
git add dev-loop/tests/test_integration.py
git commit -m "feat: implement local artifact verification"
```

---

### Task 4: Implement verify_github

**Files:**
- Modify: `tests/test_integration.py` (replace `verify_github` placeholder)

**Step 1: Implement GitHub verification**
...
def verify_github(repo_name: str) -> None:
    ...
    # Issue exists
    issues = gh_json(["issue", "list", "--repo", repo_name, "--json", "number,title,body", "--limit", "5"])
    check(
    ...
    # PR exists
    prs = gh_json(["pr", "list", "--repo", repo_name, "--state", "all", "--json", "number,title,state,baseRefName"])
    check(
...
cd /Users/yorrickjansen/work/claude-code-plugins
git add dev-loop/tests/test_integration.py
git commit -m "feat: implement GitHub state verification"
```

---

### Task 5: Run the integration test

**Step 1: Run the full test**

Run: `cd /Users/yorrickjansen/work/claude-code-plugins/dev-loop && uv run tests/test_integration.py`

This will take ~15 minutes. Monitor the output.
...
- PR comments not matching → adjust string matching
- Worktree path detection → fix glob patterns
...
cd /Users/yorrickjansen/work/claude-code-plugins
git add dev-loop/tests/test_integration.py
git commit -m "fix: address integration test issues from first run"
```
